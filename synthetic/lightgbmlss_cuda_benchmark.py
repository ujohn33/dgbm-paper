...
LIGHTGBMLSS_ROOT = REPO_ROOT / "LightGBMLSS"
if str(LIGHTGBMLSS_ROOT) not in sys.path:
    sys.path.insert(0, str(LIGHTGBMLSS_ROOT))
...
try:
    ...
    if exc.name == "pyro":
        raise SystemExit(
            "Missing dependency 'pyro'. Install 'pyro-ppl' in the HPC environment before "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Benchmark LightGBMLSS CPU vs CUDA on synthetic Gaussian data."
    )
    parser.add_argument(
        "--data-sizes",
        type=int,
    ...
    parser.add_argument("--response-fn", default="exp", choices=["exp", "softplus"])
    parser.add_argument("--stabilization", default="None", choices=["None", "MAD", "L2"])
    parser.add_argument("--loss-fn", default="nll", choices=["nll", "crps"])
    parser.add_argument("--natural-grad", action="store_true")
    parser.add_argument(
        "--time-prediction",
        action="store_true",
        help="Also time parameter prediction on the validation set.",
    )
    parser.add_argument(
        "--precompute-start-values",
        action="store_true",
        help="Precompute LightGBMLSS start values outside the timed training block.",
    )
    parser.add_argument(
        "--warmup",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="CSV path for benchmark output. If omitted, a timestamped file is created.",
    )
...
def build_lss_params(args: argparse.Namespace, device: str, seed: int) -> dict:
    return {
        "eta": args.eta,
...
def maybe_warmup(device: str, args: argparse.Namespace) -> None:
    ...
    _, start_values = model.dist.calculate_start_values(y_train)
    model.start_values = start_values
    params = build_lss_params(args, device=device, seed=args.base_seed)
...
def benchmark_device(
    ...
    train_set = lgb.Dataset(X_train, label=y_train, free_raw_data=False)
    valid_set = lgb.Dataset(X_valid, label=y_valid, reference=train_set, free_raw_data=False)
    params = build_lss_params(args, device=device, seed=seed)
    ...
    try:
        if args.precompute_start_values:
            start_value_begin = time.perf_counter()
            _, start_values = model.dist.calculate_start_values(y_train)
            result["start_value_seconds"] = time.perf_counter() - start_value_begin
            model.start_values = start_values
        ...
        result["best_iteration"] = model.booster.best_iteration

        if args.time_prediction:
            predict_begin = time.perf_counter()
            pred = model.predict(X_valid, pred_type="parameters")
...
def build_output_path(args: argparse.Namespace) -> Path:
    ...
    default_dir = REPO_ROOT / "results" / "synthetic" / "lightgbmlss_cuda_benchmark"
        ...
        for n_samples in args.data_sizes:
            ...
            for device, result in device_results.items():
                row = {
                    ...
                    "speedup_vs_cpu": speedup if device == "cuda" else 1.0 if cpu_train else None,
