...
def run_single_argument(task_id):
    ...
    if natural_flag:
        try:
            with open(f'logs/openml/xgboost/natural/{mode}/{args["dataset"]}_opt_params.json') as pset:
                default_params = json.load(pset)
        except FileNotFoundError:
            pass  # Retain the previous value of default_params
    else:
        try:
            with open(f'logs/openml/xgboost/normal/{mode}/{args["dataset"]}_opt_params.json') as pset:
                default_params = json.load(pset)
        except FileNotFoundError:
            pass  # Retain the previous value of default_params
